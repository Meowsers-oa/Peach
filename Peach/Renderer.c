...
void mRendererFlush(mContext *ctx) {
    ...
    mSetUniformMat4(ctx, "uView", ctx->renderer.viewMatrix);
    mSetUniformMat4(ctx, "uProjection", ctx->renderer.projectionMatrix);
    mSetUniformMat4(ctx, "uModel", ctx->renderer.modelMatrix);

    for (unsigned int i = 0; i < ctx->renderer.textureSlotIndex; i++) {
        glActiveTexture(GL_TEXTURE0 + i);
        glBindTexture(GL_TEXTURE_2D, ctx->renderer.textureSlots[i]);
    }
    ...
    mSetUniformIntArray(ctx, "uTextures", samplers, MAX_TEXTURE_SLOTS);

    GLsizeiptr vertexSize = (GLsizeiptr)((uint8_t*)ctx->renderer.vertexBufferPtr - (uint8_t*)ctx->renderer.vertexBuffer);
    glBindBuffer(GL_ARRAY_BUFFER, ctx->renderer.vbo);
    glBufferSubData(GL_ARRAY_BUFFER, 0, vertexSize, ctx->renderer.vertexBuffer);

    GLsizeiptr indexSize = (GLsizeiptr)((uint8_t*)ctx->renderer.indexBufferPtr - (uint8_t*)ctx->renderer.indexBuffer);
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, ctx->renderer.ebo);
    glBufferSubData(GL_ELEMENT_ARRAY_BUFFER, 0, indexSize, ctx->renderer.indexBuffer);

    glBindVertexArray(ctx->renderer.vao);
    glDrawElements(GL_TRIANGLES, (GLsizei)ctx->renderer.indexCount, GL_UNSIGNED_INT, NULL);
    glBindVertexArray(0);
...
void mAddVertices(mContext* ctx, const mVertex* vertices, unsigned int count) {
    ...
    for (unsigned int i = 0; i < count; i++) {
        ...
        if (foundIndex != -1) {
            if (ctx->renderer.indexCount >= MAX_BATCH_INDICES) {
                mRendererFlush(ctx);
                // Flush resets batch and hash table, re-insert this vertex
                bucket = hashVertex(&v) & VERTEX_HASH_MASK;
                unsigned int newIdx = ctx->renderer.vertexCount;
                *ctx->renderer.vertexBufferPtr++ = v;
                ctx->renderer.vertexCount++;
                *ctx->renderer.indexBufferPtr++ = newIdx;
            ...
            if (ctx->renderer.vertexCount >= MAX_BATCH_VERTICES || ctx->renderer.indexCount >= MAX_BATCH_INDICES) {
                mRendererFlush(ctx);
                bucket = hashVertex(&v) & VERTEX_HASH_MASK;
            }
...
void mAddVerticesIndexed(mContext* ctx, const mVertex* vertices, unsigned int vertexCount, const unsigned int* indices, unsigned int indexCount) {
    if (!ctx || !vertices || vertexCount == 0 || !indices || indexCount == 0) return;

    if (!ctx->renderer.isBatching) {
        ctx->renderer.isBatching = 1;
    }

    if (ctx->renderer.vertexCount + vertexCount > MAX_BATCH_VERTICES ||
...
void mAddQuad3DTextured(mContext* ctx, vec3 center, vec2 size, vec3 normal, const mTexture* texture, Color tint) {
    ...
    glm_vec3_cross(norm, tempUp, right);
    glm_vec3_normalize(right);
    glm_vec3_cross(right, norm, up);
